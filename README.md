...
```bash
curl -s "http://127.0.0.1:5000/suggest?q=linkin"
```
...
Строит очередь вокруг трека (`ytmusicapi.get_watch_playlist`, по умолчанию `radio=1`).

```bash
curl -s "http://127.0.0.1:5000/radio?videoId=BLZWkjBXfN8"
curl -s "http://127.0.0.1:5000/radio?videoId=BLZWkjBXfN8&limit=25&radio=1"
```
...
```json
{
  "videoId": "BLZWkjBXfN8",
  "playlistId": "RDAMVM...",
  "tracks": [ /* тот же формат трека, что у /search */ ]
}
```

Параметры:

- `videoId` (обязательный)
- `limit` — сколько треков минимум запросить (1–50, default 25)
- `radio=1` (default) — radio-очередь; `radio=0` — обычный watch playlist

Первый трек часто совпадает с seed `videoId` — на фронте можно отфильтровать.

### Публичный плейлист

Auth не нужен. `id` — `PL…` или `VLPL…` (префикс `VL` снимается).

```bash
curl -s "http://127.0.0.1:5000/playlist?id=PL3A_1s_Z8MQbYIvki-pbcerX8zrF4U8zQ&limit=50"
```
